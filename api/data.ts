import type { Company, Contact, Deal, Interaction, InteractionLink } from '../types';
import { DealStage, InteractionType, Sentiment } from '../types';

// --- MOCK DATABASE BASED ON CORE DATA MODEL ---
// This file simulates your secure Google Cloud SQL backend.
// In a real app, these functions would query your database.
...
export const getDeals = (): Promise<Deal[]> => {
    ...
    return new Promise(resolve => {
        setTimeout(() => {
            console.log("API: Responded with deals.");
            resolve(MOCK_DEALS);
        }, API_LATENCY);
    });
};

export const getInteractionsForDeal = (dealId: string): Promise<Interaction[]> => {
    console.log(`API: Fetching interactions for deal ${dealId}...`);
    return new Promise(resolve => {
        setTimeout(() => {
            ...
            console.log(`API: Responded with ${interactions.length} interactions.`);
            resolve(interactions);
        }, API_LATENCY * 1.5); // make this one a bit slower
    });
};
