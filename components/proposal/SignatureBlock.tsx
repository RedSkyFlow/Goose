...
export const SignatureBlock: React.FC<SignatureBlockProps> = ({ proposal, onProposalUpdate, finalValue, selectedItemIds }) => {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold flex items-center mb-4 text-foreground/90">
        <SignatureIcon className="w-6 h-6 mr-3 text-secondary" />
      ...
      <div className="bg-background/50 p-4 rounded-lg border border-primary/50">
        ...
        <div className="flex items-center mb-4">
          ...
          />
          <label htmlFor="consent" className="ml-2 block text-sm text-foreground/90">
        ...
        <button
          ...
          className="w-full bg-secondary hover:opacity-90 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center transition-all duration-200 disabled:bg-secondary/50 disabled:cursor-not-allowed"
        >
          {isLoading ? (
